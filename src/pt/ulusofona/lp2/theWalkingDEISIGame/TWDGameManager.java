    ...
    public int getElementId(int x, int y){
     for(int i=0; i < todosZombies.size();i++){
        ...
        for(int i=0; i < todosEquipamentos.size();i++){
            if(todosEquipamentos.get(i).x==x && todosEquipamentos.get(i).y==y){
                return todosEquipamentos.get(i).id;
            }
        }
        for(int i=0; i < todasCriaturas.size(); i++){
            if(todasCriaturas.get(i).y==y && todasCriaturas.get(i).x==x){
                return todasCriaturas.get(i).id;
            }
        }
         return 10;
    }
